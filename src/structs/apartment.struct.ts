...
export const validateApartmentAdminQuery = (
  ...
  if (apartmentStatus && !validStatuses.includes(String(apartmentStatus))) {
    throw new BadRequestError("? íš¨?˜ì? ?Šì? ?„íŒŒ???íƒœ?…ë‹ˆ??");
  }
  if (page && isNaN(Number(page))) throw new BadRequestError("page???«ì?¬ì•¼ ?©ë‹ˆ??");
  if (limit && isNaN(Number(limit))) throw new BadRequestError("limit?€ ?«ì?¬ì•¼ ?©ë‹ˆ??");
